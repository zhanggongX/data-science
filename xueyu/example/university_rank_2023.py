...
def get_page(url):
    ...
    per_universities_re = re.compile(
        '.*univNameCn:"(.*)",univNameEn:(.*?),.*univTags:(.*],?),.*univCategory:(.*),province:(.*),score:(.*),ranking:(.*),rankChange.*indData:{"411":(.*),"412":(.*),"413":(.*),"414":(.*),"415":(.*),"416":(.*),"417":(.*),"418":(.*),"419":(.*),"420":(.*)}',
        re.S)
...
if __name__ == '__main__':
    # 2023
    path = 'https://www.shanghairanking.cn/_nuxt/static/1723540750/rankings/bcur/202311/payload.js'
